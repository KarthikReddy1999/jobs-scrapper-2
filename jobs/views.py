...
def status_api(request):
    ...
    elif status == ScraperState.STATUS_RUNNING:
        status = ScraperState.STATUS_STOPPED
    total_kw = len(settings.KEYWORDS)
    kw_done = state.keyword_index
    if status == ScraperState.STATUS_IDLE and kw_done >= total_kw:
        kw_done = total_kw
    elif running:
...
def jobs_clear_all(request):
    if is_scraper_running():
        return JsonResponse({"ok": False, "message": "Stop scraper before clearing jobs"})
